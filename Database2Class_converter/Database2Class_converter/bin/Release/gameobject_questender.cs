...
namespace WoWAppVisual.Database
{
	class Gameobject_questender
	{
		...
		public Gameobject_questender(int _id, int _quest)
		{
			id = _id;
			quest = _quest;
			updateOldValues();
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM gameobject_questender WHERE id = @id AND quest = @quest;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE gameobject_questender SET id = @newid, quest = @newquest WHERE id = @id AND quest = @quest;";
