...
{
	class Areatrigger_involvedrelation
	{
		...
		public Areatrigger_involvedrelation(int _id, int _quest)
		{
			id = _id;
			quest = _quest;
			updateOldValues();
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM areatrigger_involvedrelation WHERE id = @id AND quest = @quest;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE areatrigger_involvedrelation SET id = @newid, quest = @newquest WHERE id = @id AND quest = @quest;";
