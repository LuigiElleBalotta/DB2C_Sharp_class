...
namespace WoWAppVisual.Database
{
	class Creature_questender
	{
		...
		public Creature_questender(int _id, int _quest)
		{
			id = _id;
			quest = _quest;
			updateOldValues();
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM creature_questender WHERE id = @id AND quest = @quest;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE creature_questender SET id = @newid, quest = @newquest WHERE id = @id AND quest = @quest;";
