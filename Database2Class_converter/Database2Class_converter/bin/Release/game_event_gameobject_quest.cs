...
{
	class Game_event_gameobject_quest
	{
		...
		public Game_event_gameobject_quest(int _eventEntry, int _id, int _quest)
		{
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM game_event_gameobject_quest WHERE eventEntry = @eventEntry AND id = @id AND quest = @quest;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE game_event_gameobject_quest SET eventEntry = @neweventEntry, id = @newid, quest = @newquest WHERE eventEntry = @eventEntry AND id = @id AND quest = @quest;";
