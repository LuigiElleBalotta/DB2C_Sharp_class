...
{
	class Player_factionchange_quests
	{
		...
		public Player_factionchange_quests(int _alliance_id, int _horde_id)
		{
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM player_factionchange_quests WHERE alliance_id = @alliance_id AND horde_id = @horde_id;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE player_factionchange_quests SET alliance_id = @newalliance_id, horde_id = @newhorde_id WHERE alliance_id = @alliance_id AND horde_id = @horde_id;";
