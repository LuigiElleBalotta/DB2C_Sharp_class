...
namespace WoWAppVisual.Database
{
	class Event_scripts
	{
		...
		public Event_scripts(int _id, int _delay, int _command, int _datalong, int _datalong2, int _dataint, float _x, float _y, float _z, float _o)
		{
			id = _id;
			delay = _delay;
			command = _command;
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM event_scripts WHERE id = @id AND delay = @delay AND command = @command AND datalong = @datalong AND datalong2 = @datalong2 AND dataint = @dataint AND x = @x AND y = @y AND z = @z AND o = @o;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE event_scripts SET id = @newid, delay = @newdelay, command = @newcommand, datalong = @newdatalong, datalong2 = @newdatalong2, dataint = @newdataint, x = @newx, y = @newy, z = @newz, o = @newo WHERE id = @id AND delay = @delay AND command = @command AND datalong = @datalong AND datalong2 = @datalong2 AND dataint = @dataint AND x = @x AND y = @y AND z = @z AND o = @o;";
