...
namespace WoWAppVisual.Database
{
	class Game_event_gameobject
	{
		...
		public Game_event_gameobject(int _eventEntry, int _guid)
		{
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM game_event_gameobject WHERE eventEntry = @eventEntry AND guid = @guid;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE game_event_gameobject SET eventEntry = @neweventEntry, guid = @newguid WHERE eventEntry = @eventEntry AND guid = @guid;";
