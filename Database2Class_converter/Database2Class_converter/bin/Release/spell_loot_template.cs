...
namespace WoWAppVisual.Database
{
	class Spell_loot_template
	{
		...
		public Spell_loot_template(int _Entry, int _Item, int _Reference, float _Chance, int _QuestRequired, int _LootMode, int _GroupId, int _MinCount, int _MaxCount, string _Comment)
		{
			Entry = _Entry;
			Item = _Item;
		...
		public void delete()
		{
			...
			cmd.CommandText = "DELETE FROM spell_loot_template WHERE Entry = @Entry AND Item = @Item AND Reference = @Reference AND Chance = @Chance AND QuestRequired = @QuestRequired AND LootMode = @LootMode AND GroupId = @GroupId AND MinCount = @MinCount AND MaxCount = @MaxCount AND Comment = @Comment;";
		...
		public void update()
		{
			conn.Open();
			MySqlCommand cmd = new MySqlCommand("", conn);
			cmd.CommandText = "UPDATE spell_loot_template SET Entry = @newEntry, Item = @newItem, Reference = @newReference, Chance = @newChance, QuestRequired = @newQuestRequired, LootMode = @newLootMode, GroupId = @newGroupId, MinCount = @newMinCount, MaxCount = @newMaxCount, Comment = @newComment WHERE Entry = @Entry AND Item = @Item AND Reference = @Reference AND Chance = @Chance AND QuestRequired = @QuestRequired AND LootMode = @LootMode AND GroupId = @GroupId AND MinCount = @MinCount AND MaxCount = @MaxCount AND Comment = @Comment;";
